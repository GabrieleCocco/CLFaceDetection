...
void AgroupRectangles(CV_OUT CV_IN_OUT vector<cv::Rect>& rectList, int groupThreshold, double eps=0.2);
void AgroupRectangles(CV_OUT CV_IN_OUT vector<cv::Rect>& rectList, CV_OUT vector<int>& weights, int groupThreshold, double eps=0.2);
void AgroupRectangles( vector<cv::Rect>& rectList, int groupThreshold, double eps, vector<int>* weights, vector<double>* levelWeights);
void AgroupRectangles(vector<cv::Rect>& rectList, vector<int>& rejectLevels,
...
{
public:
    ASimilarRects(double _eps) : eps(_eps) {}
...
void AgroupRectangles(vector<cv::Rect>& rectList, int groupThreshold, double eps, vector<int>* weights, vector<double>* levelWeights)
{
    ...
    vector<int> labels;
    int nclasses = partition(rectList, labels, ASimilarRects(eps));
    ...
    for( i = 0; i < nclasses; i++ )
    {
        ...
        rrects[i] = cv::Rect(r.x*s > INT_MAX ? INT_MAX : (int)(r.x*s),
                         r.y*s > INT_MAX ? INT_MAX : (int)(r.y*s),
                         r.width*s > INT_MAX ? INT_MAX : (int)(r.width*s),
                         r.height*s > INT_MAX ? INT_MAX : (int)(r.height*s));
    }
    ...
    for( i = 0; i < nclasses; i++ )
    {
        ...
        for( j = 0; j < nclasses; j++ )
        {
            ...
            int dx = r2.width * eps > INT_MAX ? INT_MAX : (int)(r2.width * eps);
            int dy = r2.height * eps > INT_MAX ? INT_MAX : (int)(r2.height * eps);
            if( i != j &&
               r1.x >= r2.x - dx &&
               r1.y >= r2.y - dy &&
...
void AgroupRectangles(vector<cv::Rect>& rectList, int groupThreshold, double eps)
{
    AgroupRectangles(rectList, groupThreshold, eps, 0, 0);
}

void AgroupRectangles(vector<cv::Rect>& rectList, vector<int>& weights, int groupThreshold, double eps)
{
    AgroupRectangles(rectList, groupThreshold, eps, &weights, 0);
}
//used for cascade detection algorithm for ROC-curve calculating
void AgroupRectangles(vector<cv::Rect>& rectList, vector<int>& rejectLevels, vector<double>& levelWeights, int groupThreshold, double eps)
{
    AgroupRectangles(rectList, groupThreshold, eps, &rejectLevels, &levelWeights);
}
                                    ...
                                    double scale )
{
    ...
    ElapseTime2 precompute_time;
    precompute_time.start();
                           ...
                           CvPoint pt, int start_stage )
{
    ...
    //ElapseTime t;
    //t.start();
    int val = cvRunHaarClassifierCascadeSum(_cascade, pt, stage_sum, start_stage);
    //printf("Subwindow time %4.4f\n", t.get());
    return val;
}
                          ...
                          CvSize minSize, CvSize maxSize, bool outputRejectLevels )
{
    ...
    else
    {
        ...
        for( ; n_factors-- > 0; factor *= scaleFactor )
        {
            ...
            if( findBiggestObject && !allCandidates.empty() && scanROI.area() == 0 )
            {
                ...
                AgroupRectangles(rectList, std::max(minNeighbors, 1), GROUP_EPS);
    ...
    if( minNeighbors != 0 || findBiggestObject )
    {
        ...
        {
            AgroupRectangles(rectList, rejectLevels, levelWeights, minNeighbors, GROUP_EPS );
        }
        else
        {
            AgroupRectangles(rectList, rweights, std::max(minNeighbors, 1), GROUP_EPS);
        }
    }
    else
